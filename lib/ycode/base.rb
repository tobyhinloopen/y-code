...
module Ycode
	class Base
		def initialize
			...
			global_string_scanner.register_replacement /(?:\A|\^(\d))([\s\S]*?)(?=\Z|\^\d)/, ->(_, color_id, content) {
			...
			{ /_(.+)_/ => ->(_, content) { %(<strong>#{CGI.escapeHTML(content)}</strong>) },
			  ...
			  /~~~~\r\n(.+?)\r\n([\s\S]*?)\r\n~~~~/ => ->(_, quotee, content) {
			  	%(<blockquote><span class="User Name">#{CGI.escapeHTML(quotee)}</span>#{global_string_scanner.replace(content)}</blockquote>)
			  },
			  "<" => "&lt;",
			  ">" => "&gt;",
			  "&" => "&amp;",
